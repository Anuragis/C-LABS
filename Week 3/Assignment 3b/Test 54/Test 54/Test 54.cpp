...
void print_primes(bool input_arr[]) {
	...
	int check_number_count = 0; // flag to check numbers per line in output.

	for (int i = 2;i < MAX_NUMBER + 1;i++) {
		if (input_arr[i]) {
			if (check_number_count == NUMBERS_PER_LINE) {
				//If count of printed numbers on current line equals 20, break the line.
				cout << endl;
				check_number_count = 0;//Reset number counter.
			}
			cout << setw(4) << i;
			check_number_count++;
		}
